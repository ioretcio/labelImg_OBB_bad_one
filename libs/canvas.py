...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        if Qt.LeftButton & ev.buttons(): # ev.buttons is mouse buttons mask (https://doc.qt.io/qt-5/qt.html#MouseButton-enum)
            if self.selectedVertex() and Qt.ControlModifier != int(mods):
                self.boundedMoveVertex(pos)
                self.shapeMoved.emit()
                self.repaint()
            elif self.selectedVertex() and Qt.ControlModifier == int(mods):
        ...
        for shape in reversed([s for s in self.shapes if self.isVisible(s)]):
            ...
            index = shape.nearestVertex(pos, int(self.epsilon/self.scale))
    ...
    def mousePressEvent(self, ev):
        ...
        if ev.button() == Qt.MidButton or int(mods) == 134217728:
            self.drag = True
            self.dragPos = (ev.x(), ev.y())
