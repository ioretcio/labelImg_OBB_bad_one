...
DEFAULT_SELECT_FILL_COLOR = QColor(0, 128, 255, 30)
...
class Shape(object):
    ...
    def drawVertex(self, path, i):
        ...
        if shape == self.P_SQUARE:
            path.addRect(point.x() - d / 3.0, point.y() - d / 3.0, d, d)
    ...
    def drawOrigin(self, path):
        d = self.point_size / self.scale
        path.addEllipse(QPoint(self.origin[0], self.origin[1]), d / 3.0, d / 3.0)
